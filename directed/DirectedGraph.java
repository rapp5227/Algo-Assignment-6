 ...
 *
 * @author Abby Westphal and Chase Rapp
 */
...
public class DirectedGraph {
	...
	public boolean isAcyclic()
	{
		...
		ListIterator<DirectedGraphNode> nodeList = this.nodes.listIterator();
		ListIterator<DirectedGraphNode> outs;
		...
		while(nodeList.hasNext())
		{
			...
			outs = start.getOutgoingNodes().listIterator();

			while(outs.hasNext())
			{
				if(outs.next().getPostNumber() > start.getPostNumber())
					return false;
			}
		}
	...
	private void explore(int i,boolean[] visited)
	{
		visited[i] = true;

		DirectedGraphNode current = findNode(i);

		current.setPreNumber(clock);
		clock++;

		ListIterator<DirectedGraphNode> outs = current.getOutgoingNodes().listIterator();

		while(outs.hasNext())
		{
			DirectedGraphNode next = outs.next();

			if(!visited[next.getData()])
				explore(next.getData(),visited);
		}

		current.setPostNumber(clock);
		clock++;
